...
## Are expression data normal or log normally distributed?


```{r}


check_reg_vs_log = function(vals, min_mean_val, title) {
  ...
  for (gene in genes) {
    ...
    if (var(gene_vals) > 1 && mean(gene_vals) >= min_mean_val) {
      
      gene_vals = gene_vals + rnorm(length(gene_vals))/1000 # break ties only
      log_gene_vals = log(gene_vals + 1)
      
      r = ks.test(x=gene_vals, y = "pnorm", mean(gene_vals), sd(gene_vals))
      reg_stats_vec = c(reg_stats_vec, r$statistic)
    
      rlog = ks.test(x=log_gene_vals, y="pnorm", mean(log_gene_vals), sd(log_gene_vals))
      log_stats_vec = c(log_stats_vec, rlog$statistic)
      }
  }
  plot(density(reg_stats_vec), col='red', main=title)
  points(density(log_stats_vec), col='blue')
  
}

check_reg_vs_log(vals=GM_gene_vals, min_mean_val=1000, title='GM')
check_reg_vs_log(vals=H1_gene_vals, min_mean_val=1000, title='H1')
...
for (gene in genes) {
  ...
  tested_actual_GM_vals = GM_vals
  tested_actual_H1_vals = H1_vals
